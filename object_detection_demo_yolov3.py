...
# out = cv2.VideoWriter('output.avi', cv2.VideoWriter_fourcc(*'MJPG'), 20.0, (640, 720))
# left_top = (205, 350)
# bottom_right = (264, 380)
# random_boxes = [[(170, 260), (375, 567)], [(168, 262), (373, 564)], [(171, 262), (377, 565)], [(171, 262), (372, 565)]]
...
pts = np.array([[223,414],[604,279],[976, 305],[853,521]], np.int32)
pts = pts.reshape((-1,1,2))
color_n, color_c, color_h, color_w, color_exec_net, color_input_blob, color_out_blob = load_xml_bin('/home/kelvin/Accubits/fujairah/model/vehicle_attributes.xml')
...
def check_movement(cent, a1, b1, p_id, cam_id, count_type, buff_dict):
    ...
    if cam_id in buff_dict:
        if p_id in buff_dict[cam_id]:
            ...
            if int(time()) - out['timestamp'] > 9000:
                buff_dict[cam_id][p_id] = {'timestamp': int(time()), 'd': Dir}
            else:
                if out['d'] != Dir:
                    buff_dict[cam_id][p_id] = {'timestamp': int(time()), 'd': Dir}
                    flag = True
                else:
                    buff_dict[cam_id][p_id] = {'timestamp': int(time()), 'd': out['d']}
        else:
            buff_dict[cam_id][p_id] = {'timestamp': int(time()), 'd': Dir}
    else:
...
def main():
    ...
    while cap.isOpened():
        ...
        if totalFrames % 5 == 0:
            ...
            for obj in objects:
                ...
                if int(det_label) in coi:
                    ...
                    in_frame = frame[abs(obj['ymin']):abs(obj['ymax']), abs(obj['xmin']):abs(obj['xmax'])]
                    in_frame = cv2.resize(in_frame, (color_w, color_h))
                    in_frame = in_frame.transpose((2, 0, 1))  # Change data layout from HWC to CHW
                    in_frame = in_frame.reshape((color_n, color_c, color_h, color_w))
                    color_exec_net.start_async(request_id=cur_request_id, inputs={color_input_blob: in_frame})
                    if color_exec_net.requests[cur_request_id].wait(-1) == 0:
                        res = color_exec_net.requests[cur_request_id].outputs
                        color_of_roi = ['white', 'gray', 'yellow', 'red', 'green', 'blue', 'black'][np.argmax(res['color'][0])]
                        type_of_roi = ['car', 'bus', 'truck', 'van'][np.argmax([res['type'][0]])]
                        cv2.putText(frame, color_of_roi + ' ' + coi_dict[int(det_label)], ((obj['xmax'] + obj['xmin'])//2, (obj['ymax'] + obj['ymin'])//2), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, (0, 0, 255), 1)
        ...
        for (objectID, data) in objects.items():
            ...
            cv2.rectangle(frame, (objectRect[0], objectRect[1]), (objectRect[2], objectRect[3]), (255, 255, 0))
